...
#   PINNED TO (see THE STEP MAP below), or when CI runs `make <target>` or an
#   ancestor whose prerequisite closure contains it.
...
while IFS= read -r target; do
	...
	via=anchors
	(make_invokes "$target" || make_invokes_ancestor "$target") && via=make
